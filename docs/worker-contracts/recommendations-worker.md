...
> **Deprecated:** Recommendations are no longer generated by an external HTTP worker. The `RecommendationGenerationService` calls the OpenAI API directly from within the NestJS process. See [AI Inference Pipeline — Recommendation Generation](../architecture/ai-inference-pipeline.md#12-recommendation-generation) for the current architecture.
