...
export class Migration20260218150103_AddQuestionnaireDraft extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table "questionnaire_draft" ("id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, "respondent_id" varchar(255) not null, "questionnaire_version_id" varchar(255) not null, "faculty_id" varchar(255) not null, "semester_id" varchar(255) not null, "course_id" varchar(255) null, "answers" jsonb not null, "qualitative_comment" text null, constraint "questionnaire_draft_pkey" primary key ("id"));`);
    ...
    // Partial unique index to handle soft deletes - only enforce uniqueness on non-deleted records
    // Two separate indexes to handle NULL course_id properly (NULLs are treated as distinct in regular unique constraints)
    this.addSql(`create unique index "questionnaire_draft_unique_active_with_course" on "questionnaire_draft" ("respondent_id", "questionnaire_version_id", "faculty_id", "semester_id", "course_id") where "deleted_at" is null and "course_id" is not null;`);
    this.addSql(`create unique index "questionnaire_draft_unique_active_without_course" on "questionnaire_draft" ("respondent_id", "questionnaire_version_id", "faculty_id", "semester_id") where "deleted_at" is null and "course_id" is null;`);
