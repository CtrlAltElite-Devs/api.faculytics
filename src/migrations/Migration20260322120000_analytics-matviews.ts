...
export class Migration20260322120000 extends Migration {
  override async up(): Promise<void> {
    // 1. Covering index for LATERAL join performance on sentiment_result
    this.addSql(
    ...
    // 2. mv_faculty_semester_stats — one row per (faculty, semester, department)
    // F1 fix: topic counts pre-aggregated in CTE to prevent LATERAL fan-out inflating AVG()
    this.addSql(`
    ...
    // 3. Unique index on mv_faculty_semester_stats — required for REFRESH CONCURRENTLY
    this.addSql(
    ...
    // 4. Lookup index on mv_faculty_semester_stats
    this.addSql(
      `CREATE INDEX idx_mv_fss_dept_semester
        ON mv_faculty_semester_stats (department_code_snapshot, semester_id);`,
    );

    // 5. mv_faculty_trends — one row per (faculty, department) across all semesters
    this.addSql(`
    ...
    // 6. Unique index on mv_faculty_trends — required for REFRESH CONCURRENTLY
    this.addSql(
      `CREATE UNIQUE INDEX uq_mv_faculty_trends
        ON mv_faculty_trends (faculty_id, department_code_snapshot);`,
    );

    // 7. Lookup index on mv_faculty_trends
    this.addSql(
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`DROP INDEX IF EXISTS idx_sr_submission_processed;`);
  }
}
